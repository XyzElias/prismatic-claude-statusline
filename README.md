...
The script creates its config file (`~/.claude/statusline_config.yml`) automatically on first run, so there is nothing else to set up. Want the rounded Powerline pill caps in the hero image? See the [Nerd Font guide](docs/nerd-fonts.md) — or stay on the default `unicode` style, which needs no special font.

> **Optional — the visual editor:** the command above downloads only `statusline.js` (all you need to run). To also use the in-browser config editor, grab [`config-editor.html`](config-editor.html) and open it in any browser (no server needed — just double-click it). For example:
> ```bash
> # macOS / Linux / Git Bash
...
- **Visual editor (recommended):** open [`config-editor.html`](config-editor.html) in any browser — no install, nothing to run. Drag to reorder segments, toggle them on/off, pick colors and per-model gradients, and watch a **live preview** update as you go. Click **Copy YAML** and save it to `~/.claude/statusline_config.yml`.
...
Issues and pull requests are welcome — color themes, extra segments, and platform fixes especially. The whole thing is one readable file (`statusline.js`); the images in this README are generated by [`tools/render-svg.js`](tools/render-svg.js), so they stay in sync with the real output.
